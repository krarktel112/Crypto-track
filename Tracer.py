...
# Expanded registry covering Centralized Exchanges (CEX) and DeFi platforms
DEFI_CEX_REGISTRY = {
...
def log_and_print(text):
    """Prints output to the console and queues it for the evidence file."""
    print(text)
    EVIDENCE_LOG.append(text)
    ...
    try:
        ...
        for log in reversed(logs):
            ...
            if len(topics) == 3 and topics[0].lower().startswith("0xddf252ad"):
                potential_recipient = "0x" + topics[-1][-40:]
...
def get_all_outbound_movements(wallet_address, start_timestamp, end_timestamp):
    ...
    # Layer 2: Internal Smart Contracts
...
def trace_forensic_tree(current_wallet, start_timestamp, end_timestamp, current_depth, max_depth, visited=None):
    ...
    log_and_print(f"\n[Depth {current_depth}] Auditing ALL outbound pathways for node: {current_wallet}")
    log_and_print(f"      Explorer Verification URL: https://etherscan.io{current_wallet}")
    
    if current_wallet in DEFI_CEX_REGISTRY and "Hot Wallet" in DEFI_CEX_REGISTRY[current_wallet]:
        log_and_print(f"🛑 CRITICAL ENDPOINT REACHED: Funds terminated at an exchange portal -> {DEFI_CEX_REGISTRY[current_wallet]}")
        return
    ...
    if not hops and current_depth == 0:
        log_and_print("   ↳ (No asset footprints found leaving this node. Funds are holding stagnant inside this wallet.)")
    ...
    for hop in hops:
        ...
        if dest in DEFI_CEX_REGISTRY and "Hot Wallet" in DEFI_CEX_REGISTRY[dest]:
            log_and_print(f"   ↳ 🚨 CENTRALIZED EXCHANGE TARGET EXPOSED: {date_str}")
            log_and_print(f"      • Destination Address : {dest}")
            log_and_print(f"      • Exchange Entity     : {DEFI_CEX_REGISTRY[dest]}")
            log_and_print(f"      • Value Exfiltrated  : {hop['val']:.2f} {hop['asset']}")
            log_and_print(f"      • Subpoena Tx Hash    : {hop['hash']}")
            log_and_print(f"      • Direct Explorer Link: https://etherscan.io{hop['hash']}")
            continue
            
        elif dest in DEFI_CEX_REGISTRY:
            router_name = DEFI_CEX_REGISTRY[dest]
            log_and_print(f"   ↳ 🔄 DEX SWAP/BRIDGE INTERCEPTED: {date_str} via Router Pool [{router_name}]")
            log_and_print(f"      • Interaction Tx Hash : {hop['hash']}")
            log_and_print("      🔍 Decoding event transfer logs to retrieve post-swap target...")
            
            time.sleep(0.3)
            real_recipient = trace_internal_swap_recipient(hop['hash'], current_wallet)
            
            if real_recipient:
                log_and_print(f"      🎯 Trail recovered! Post-swap funds forwarded out to: {real_recipient}")
                trace_forensic_tree(real_recipient, hop['ts'], end_timestamp, current_depth + 1, max_depth, visited)
            else:
                log_and_print("      ↳ (Liquidity pool exit address matched a nested compiler router or multi-sig vault; trail obscured.)")
                
        else:
            log_and_print(f"   ↳ FLIGHT DETECTED: {date_str}")
            log_and_print(f"      • Moved: {hop['val']:.4f} {hop['asset']} -> Out to Private Node: {dest}")
            log_and_print(f"      • Transaction Tx Hash : {hop['hash']}")
...
def main():
    ...
    log_and_print("=======================================================================")
    log_and_print("             OFFICIAL CRYPTO FRAUD FORENSIC EVIDENCE LOG               ")
    log_and_print("=======================================================================")
    log_and_print(f"Generated On (UTC)       : {current_time_str}")
    log_and_print(f"Scam Target Wallet Node   : {START_WALLET}")
    log_and_print(f"Cash App Deposit Timeline : {CASH_APP_DEPOSIT_TIME}")
    log_and_print(f"Analysis Window Cap       : Up to Current Block State ({current_time_str})")
    log_and_print("=======================================================================\n")
    
    trace_forensic_tree(START_WALLET, funding_timestamp, target_end_timestamp, current_depth=0, max_depth=MAX_HOPS)
    
    log_and_print("\n=======================================================================")
    log_and_print("                    END OF FORENSIC DATA EVIDENCE                      ")
    log_and_print("=======================================================================")
    
    # Save compilation directly to disk
    try:
        with open("scam_forensic_evidence.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(EVIDENCE_LOG))
        print("\n💾 Success! Professional forensic file generated: scam_forensic_evidence.txt")
